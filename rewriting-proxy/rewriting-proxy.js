...
var eventHandlerAttributeNames = [
  "onabort",
  "onafterprint",
  "onbeforeprint",
  "onbeforeunload",
...
function walkDOM(node, url, rewriteFunc, headerHTML, headerURLs, options) {
  ...
  if (node.childNodes && node.childNodes.length) {
    ...
    var childNodes = options.onNodeVisited
      ? node.childNodes.slice(0)
      : node.childNodes;
...
function start(options) {
  assert(
    options.rewriter,
    "must provide rewriter function in options.rewriter"
  );
    ...
    var proxyRequest = http.request(
      ...
      function (proxy_response) {
        ...
        if (noInst) {
          ...
          tp.match(/JavaScript/i) ||
          (tp.match(/text/i) && url_path.match(/\.js$/i))
        ) {
          tp = "JavaScript";
        ...
        proxy_response.on("end", function () {
          ...
          if (output) {
            proxy_response.headers["content-length"] = Buffer.byteLength(
              output,
              "utf-8"
            );
            response.writeHead(
...
function rewriteExternalScript(src, url, rewriteFunc) {
  var metadata = {
    type: "script",
    inline: false,
    url: url,
  ...
  return rewriteScript(src, metadata, rewriteFunc);
}

exports.start = start;
exports.rewriteHTML = rewriteHTML;
exports.rewriteExternalScript = rewriteExternalScript;
