...
function startJalangiProxy(port) {
  proxy.start({
    headerHTML: instUtil.getHeaderCode(),
    rewriter: rewriter,
    port: port,
  });
...
var parser = new ArgumentParser({
  addHelp: true,
  description: "Jalangi instrumenting proxy server",
});
parser.addArgument(["-o", "--outputDir"], {
  help: "output parent directory for instrumented scripts",
});
parser.addArgument(["-i", "--ignoreInline"], {
  help: "ignore all inline scripts",
  nargs: "?",
  defaultValue: false,
  constant: true,
});
